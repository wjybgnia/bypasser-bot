...
case "$1" in
    ...
    setup-auto)
        ...
        echo "*/5 * * * * /home/scriptblox-bot/auto-update.sh" | crontab -
        echo "✅ Auto-update scheduled every 5 minutes"
        ;;
    *)
        echo "🤖 ScriptBlox Discord Bot Management"
        echo "Usage: $0 {start|stop|restart|status|logs|update|deploy|monitor|backup|webhook|setup-auto}"
        echo ""
        echo "Commands:"
        ...
        echo "  webhook    - Start webhook server for auto-updates"
        echo "  setup-auto - Setup automatic updates via cron"
        ;;
esac
